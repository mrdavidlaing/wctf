...
def save_orgmap(
    company_name: str, orgmap_yaml: str, base_path: Optional[Path] = None
) -> Dict:
    ...
    try:
        ...
        orgmap_path.parent.mkdir(parents=True, exist_ok=True)
...
def get_orgmap(company_name: str, base_path: Optional[Path] = None) -> Dict:
    ...
    try:
        ...
        if not orgmap_path.exists():
            return {"success": False, "error": f"No orgmap found for {company_name}"}

        orgmap_data = read_yaml(orgmap_path)
        orgmap = CompanyOrgMap(**orgmap_data)

        return {"success": True, "orgmap": orgmap.model_dump()}
    except Exception as e:
        return {"success": False, "error": str(e)}
